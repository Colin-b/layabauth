...
class AsyncRouteTest(JSONTestCase):
    ...
    def test_async_ns_proxy_creates_2_extra_endpoints_per_declared_endpoint(self):
        ...
        self.assert_swagger(response, {'swagger': '2.0', 'basePath': '/', 'paths': {'/foo/bar': {'get': {'responses': {
            '202': {'description': 'Computation started.', 'schema': {'$ref': '#/definitions/AsyncTaskStatusModel'},
                    'headers': {
            ...
            '/foo/modified_task_result/status/{task_id}': {
                ...
                'AsyncTaskStatusModel': {'properties': {'task_id': {'description': 'Task ' 'Id.', 'type': 'string'},
                                                        'url': {'description': 'URL when task status can be checked.',
                                                                'type': 'string'}},
    ...
    def test_health_details_without_expected_workers(self):
        ...
        self.assertEqual(details, {
            'celery:ping': {
                'componentType': 'component',
                'output': [{'celery@test2': {'pong': 'ok'}}, {'celery@test3': {'pong': 'ok'}},
                           {'celery@test2': {'pong': 'ok'}}],
                'status': 'fail',
                'time': '2018-10-11T15:05:05.663979'
            }
